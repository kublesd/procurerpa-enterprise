...
from enterprise.llm.action_cache import RedisActionCacheStore, configure_persistent_cache_store
...
def create_forge_app() -> ForgeApp:
    ...
    action_cache = RedisActionCacheStore(redis_from_url(settings.REDIS_URL, decode_responses=True))
    configure_persistent_cache_store(action_cache)
    app.agent = ForgeAgent(action_cache=action_cache)
