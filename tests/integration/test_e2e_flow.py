...
Simulates the lifecycle across 10 phases:
  ...
  2. Risk detection — deterministic procurement rules with optional LLM escalation
  ...
  9. Workflow templates — all 6 procurement templates registered
...
from enterprise.audit.sanitizer import hash_raw_value, sanitize_input
...
def _procurement_risk_context(**changes) -> RiskContext:
    values = {
...
def _dashboard_facts() -> tuple[list[tuple[SimpleNamespace, SimpleNamespace]], list[SimpleNamespace]]:
    task_rows = []
    for task in SAMPLE_TASKS:
        task_rows.append((
            SimpleNamespace(
                ...
                department_id=DEPARTMENTS["credit"][0],
                category_id="category_corporate_loan",
                risk_level="low",
            ),
            SimpleNamespace(
                task_id=task["task_id"],
    ...
    approval_rows = [SimpleNamespace(status=approval["status"]) for approval in SAMPLE_APPROVALS]
    return task_rows, approval_rows
...
class TestPhase2RiskDetection:
    """Structured procurement-risk detection."""

    @pytest.mark.asyncio
    async def test_large_purchase_triggers_high_risk(self):
        result = await detect_risk(_procurement_risk_context(total_amount_cny="500000"))
        assert result.risk_level == "high"
        assert result.findings[0].category == "amount"

    @pytest.mark.asyncio
    async def test_standard_purchase_is_low_risk(self):
        result = await detect_risk(_procurement_risk_context())
        assert result.risk_level == "low"

    @pytest.mark.asyncio
    async def test_invalid_supplier_qualification_is_critical(self):
        result = await detect_risk(_procurement_risk_context(supplier_qualified=False))
        assert result.risk_level == "critical"

    @pytest.mark.asyncio
    async def test_single_source_procurement_is_high(self):
        result = await detect_risk(_procurement_risk_context(operation_type="single_source"))
        assert result.risk_level == "high"
...
class TestPhase8DashboardStats:
    """Procurement overview and trend computation."""

    def test_overview_metrics(self):
        task_rows, approval_rows = _dashboard_facts()
        overview = compute_procurement_overview(
            task_rows,
            [],
            approval_rows,
            now=NOW,
        ...
        assert overview["completed_tasks"] == 5
        assert overview["failed_tasks"] == 2

    def test_trend_has_7_days(self):
        task_rows, _ = _dashboard_facts()
        trend = compute_procurement_trend(
            task_rows,
            days=7,
            now=NOW,
        )
...
class TestPhase9WorkflowTemplates:
    """Verify all 6 procurement workflow templates."""
...
class TestPhase10FullE2EScenario:
    ...
    async def test_full_lifecycle(self, operator_ctx, approver_ctx, risk_viewer_ctx):
        ...
        # Step 2: Detect high-risk procurement from structured deterministic inputs
        risk = await detect_risk(_procurement_risk_context(total_amount_cny="500000"))
        ...
        task_rows, approval_rows = _dashboard_facts()
        overview = compute_procurement_overview(task_rows, [], approval_rows, now=NOW)
