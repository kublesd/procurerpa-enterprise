...
class TestPlannerAgent(unittest.IsolatedAsyncioTestCase):
    async def test_fallback_plan_without_llm(self):
        ...
        assert "supplier" in plan.subtasks[1].goal.lower()
    ...
    async def test_plan_llm_failure_falls_back(self):
        ...
        plan = await planner.create_plan("Test goal")
        # Should fall back to the procurement demo plan
        assert len(plan.subtasks) >= 3
...
class TestExecutorAgent(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_real_execution_without_handler_fails_closed(self):
        executor = ExecutorAgent(action_handler=None)
        subtask = SubTask(index=0, goal="Inspect supplier", completion_condition="Evidence exists")

        result = await executor.execute_subtask(subtask, {"execution_mode": "real"})

        assert result.success is False
        assert result.simulated is False
        assert result.error_message == "real_executor_handler_missing"

    async def test_execute_with_handler_success(self):
        async def handler(goal, context):
            return {
                "success": True,
                "data": {"page": "supplier-quotes"},
                "screenshot_key": "demo/quote-page.png",
        ...
        assert result.success is True
        assert result.simulated is False
        assert result.result_data["page"] == "supplier-quotes"
        assert result.screenshot_key == "demo/quote-page.png"
        assert result.page_url == "https://supplier.example/quotes"
...
class TestCoordinatorResumption(unittest.IsolatedAsyncioTestCase):
    async def test_resume_skips_completed_subtasks(self):
        ...
        planner = PlannerAgent(llm_callable=mock_llm)
        fixed_plan = await planner.create_plan("3-step task")
        planner.create_plan = AsyncMock(return_value=fixed_plan)
        executor = ExecutorAgent(action_handler=mock_handler)
        coordinator = AgentCoordinator(planner, executor)
        ...
        assert execute_calls == ["Step B", "Step C"]
