...
class TestGetCurrentUser:
    ...
    @pytest.mark.asyncio
    async def test_valid_token_returns_context(self, monkeypatch):
        token = _make_token()
        expected = _make_user_ctx([("dept_1", "Dept 1", "operator")])

        async def load_user(_user_id: str) -> UserContext:
            return expected

        monkeypatch.setattr("enterprise.auth.dependencies._load_current_user", load_user)
    ...
    @pytest.mark.asyncio
    async def test_forged_token_returns_401(self):
        token = jwt.encode(
            {"sub": "eu_test", "org_id": "org_1", "exp": 4_102_444_800, "department_roles": [], "business_line_ids": []},
    ...
    @pytest.mark.asyncio
    async def test_disabled_user_returns_401(self, monkeypatch):
        async def load_user(_user_id: str) -> None:
            return None

        monkeypatch.setattr("enterprise.auth.dependencies._load_current_user", load_user)
        with pytest.raises(HTTPException) as exc_info:
            await get_current_user(authorization=f"Bearer {_make_token()}")
        assert exc_info.value.status_code == 401

    @pytest.mark.asyncio
    async def test_roles_are_loaded_from_server_not_token(self, monkeypatch):
        async def load_user(_user_id: str) -> UserContext:
            return _make_user_ctx([("dept_1", "Dept 1", "viewer")])

        monkeypatch.setattr("enterprise.auth.dependencies._load_current_user", load_user)
        token = _make_token(dept_roles=[("dept_1", "Dept 1", "super_admin")])
