"""Tests for the composable procurement skill library.
...
- Workflow template skill_steps: all 6 procurement templates have skill_steps
...
class TestLoginSkill(unittest.IsolatedAsyncioTestCase):
    def test_params_validation(self):
        ...
        assert p.url == "https://supplier.example.com"
...
class TestSearchAndSelectSkill(unittest.IsolatedAsyncioTestCase):
    async def test_search_and_select_with_llm(self):
        ...
        result = await skill.execute(params, context={"page": mock_page, "llm_handler": mock_llm})
        assert result.status == SkillStatus.COMPLETED
        assert result.data["selected"] == "RFQ RFQ001"
...
class TestTemplateSkillSteps(unittest.TestCase):
    ...
    def test_supplier_quote_has_login_extract_download(self):
        from enterprise.workflows.templates import SUPPLIER_QUOTE_LEDGER
        skill_names = [s.skill_name for s in SUPPLIER_QUOTE_LEDGER.skill_steps]
