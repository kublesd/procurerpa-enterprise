"""Three-layer resilient LLM caller for procurement RPA.
...
Layer 3 (Task):   After max retries, marks the procurement task NEEDS_HUMAN

This is an independent first-phase demo helper; it is not connected to a
real Skyvern Task or Action.
"""
...
def build_structured_prompt(
    ...
    prompt = (
        "You are a procurement RPA assistant handling supplier quotes, supplier details, "
        "contract terms, and procurement portal actions. You MUST respond with valid JSON "
        "matching the following procurement schema. Do NOT include credentials, tokens, "
        "bank-account numbers, or other secrets, and do NOT include any text outside the JSON object.\n\n"
        f"## Required Procurement JSON Schema\n```json\n{schema_json}\n```\n\n"
    )
...
async def call_llm_with_retry(
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            error_msg = f"Attempt {attempt + 1}: JSON parse error"
            result.errors.append(error_msg)
            logger.warning(error_msg)

        except ValidationError:
            error_msg = f"Attempt {attempt + 1}: Schema validation error"
            result.errors.append(error_msg)
            logger.warning(error_msg)

        except Exception as e:
            # Exception text can contain request data, credentials, or provider tokens.
            error_msg = f"Attempt {attempt + 1}: LLM call error — {type(e).__name__}"
            result.errors.append(error_msg)
            logger.warning(error_msg)
