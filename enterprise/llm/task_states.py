"""Extended task state machine for procurement RPA.

Adds enterprise-specific states to Skyvern's base task lifecycle:
- PENDING_APPROVAL: task blocked waiting for human approval
- NEEDS_HUMAN: quote, supplier, contract, or page action needs human review
- PAUSED: manually paused by operator

The state machine is an independent first-phase demo; it is not a Skyvern
TaskStatus adapter yet.
"""
...
VALID_TRANSITIONS: dict[str, set[str]] = {
    ...
    EnterpriseTaskStatus.NEEDS_HUMAN.value: {
        EnterpriseTaskStatus.RUNNING.value,       # human skips/completes procurement step
