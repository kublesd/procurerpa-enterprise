...
def _validate_custom_regex(name: str, value: str, pattern: str) -> ValidationError | None:
    ...
    try:
        if not re.match(pattern, value):
            return ValidationError(name, "Value does not match required pattern")
    except re.error:
        return ValidationError(name, "Invalid validation regex in template definition")
    return None
...
def validate_parameters(
    ...
    known_names = {pdef.name for pdef in param_defs}
    errors.extend(
        ValidationError(name, "Unknown template parameter")
        for name in params.keys() - known_names
    )
