...
from .schemas import DepartmentRole, UserContext


async def _load_current_user(user_id: str) -> UserContext | None:
    """Load the active user's authorization context from trusted server data."""
    async with forge_app.DATABASE.Session() as session:
        user = (await session.execute(
            select(EnterpriseUserModel).where(EnterpriseUserModel.user_id == user_id)
        )).scalar_one_or_none()
        ...
        department_rows = (await session.execute(
            select(
                UserDepartmentRoleModel.department_id,
                DepartmentModel.department_name,
                UserDepartmentRoleModel.role,
            )
            .join(DepartmentModel, DepartmentModel.department_id == UserDepartmentRoleModel.department_id)
            .where(
                UserDepartmentRoleModel.user_id == user.user_id,
                DepartmentModel.organization_id == user.organization_id,
            )
        )).all()
        business_line_ids = (await session.execute(
        ...
        procurement_category_ids = (await session.execute(
            select(UserProcurementCategoryModel.category_id)
            .join(
                ProcurementCategoryModel,
                ProcurementCategoryModel.category_id == UserProcurementCategoryModel.category_id,
            )
            .where(
                UserProcurementCategoryModel.user_id == user.user_id,
                ProcurementCategoryModel.organization_id == user.organization_id,
            )
        )).scalars().all()
        special_permissions = set((await session.execute(
            select(SpecialPermissionModel.permission_type).where(
    ...
    return UserContext(
        ...
        business_line_ids=list(business_line_ids),
        procurement_category_ids=list(procurement_category_ids),
        has_cross_org_read="cross_org_read" in special_permissions,
        has_cross_org_approve="cross_org_approve" in special_permissions,
    )
...
async def get_current_user(
    ...
    """Validate a Bearer token and resolve the active user from server data.

    Token claims establish the user identity; active status, organization, and
    roles are loaded from the enterprise tables on every request.
    ...
    user = await _load_current_user(token_user.user_id)
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
