...
from .context import TenantContext, tenant_context_for
...
async def list_tasks(user: CurrentUser) -> dict:
    ...
    The query uses the current request's server-resolved tenant scope.
    """
    ctx = tenant_context_for(user)

    async with _get_db_session() as session:
        result = await session.execute(filter_task_extensions(select(TaskExtensionModel), ctx))
...
async def diagnose_visibility(
    ...
    async with _get_db_session() as session:
        ...
        user_stmt = select(EnterpriseUserModel).where(
            ...
            EnterpriseUserModel.organization_id == admin.org_id,
        )
        ...
        dept_roles_stmt = (
            ...
            .where(
                ...
                DepartmentModel.organization_id == admin.org_id,
            )
        )
        ...
        bl_stmt = (
            ...
            .where(
                ...
                BusinessLineModel.organization_id == admin.org_id,
            )
        )
        ...
        category_rows = (await session.execute(
            ...
            .where(
                UserProcurementCategoryModel.user_id == user_id,
                ProcurementCategoryModel.organization_id == admin.org_id,
            )
        )).scalars().all()
        ...
        sp_stmt = select(SpecialPermissionModel).where(
            ...
            SpecialPermissionModel.organization_id == admin.org_id,
        )
    ...
    return {
        ...
        "procurement_category_ids": list(category_rows),
        "special_permissions": [
            {
...
def _format_ctx(ctx: TenantContext) -> dict:
    return {
        ...
        "visible_category_ids": ctx.visible_category_ids,
    }
