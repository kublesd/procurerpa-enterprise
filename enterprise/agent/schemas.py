"""Data models for the multi-agent procurement coordination system.
...
class SubTask(BaseModel):
    """A single procurement step produced by PlannerAgent."""

    subtask_id: str = Field(default_factory=lambda: f"sub_{uuid.uuid4().hex[:12]}")
    index: int = Field(description="Execution order (0-based)")
    goal: str = Field(description="What this procurement sub-task should accomplish")
...
class TaskPlan(BaseModel):
    """A complete procurement plan produced by PlannerAgent."""

    plan_id: str = Field(default_factory=lambda: f"plan_{uuid.uuid4().hex[:12]}")
    # Keep the field name for compatibility with the existing coordination API.
    navigation_goal: str = Field(description="Original procurement goal")
...
class ExecutionResult(BaseModel):
    ...
    skyvern_task_id: str | None = None
    status: str | None = None
    artifact_id: str | None = None
    duration_ms: int | None = None
    simulated: bool = Field(
        default=False,
        description="True when the demo executor ran without a real handler",
    )


class CoordinationState(BaseModel):
    """Overall state of procurement Planner-Executor coordination."""
    ...
    version: int = Field(default=0, description="Persisted snapshot version; zero means not yet saved")
