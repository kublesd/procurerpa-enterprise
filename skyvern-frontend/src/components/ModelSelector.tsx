...
const constants = {
  SkyvernOptimized: "ProcureRPA Optimized",
} as const;
  ...
  return (
    <div className="flex items-center justify-between">
      <div className="flex gap-2">
        <Label className="text-xs font-normal" style={{ color: "var(--procurerpa-text-secondary)" }}>Model</Label>
            ...
            {choices.map((m) => (
              <SelectItem key={m} value={m}>
                {m === constants.SkyvernOptimized ? (
                  <span>ProcureRPA Optimized ✨</span>
                ) : (
                  m
                )}
              </SelectItem>
