    ...
    >
      ...
      >
        <div className="space-y-2">
          <div className="flex justify-between">
            <div className="flex gap-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                <span className="flex gap-2 text-xs" style={{ color: "var(--procurerpa-text-muted)" }}>
                  {block.label}
                </span>
                {isFinallyBlock && (
          ...
          {block.description ? (
            <div className="text-xs" style={{ color: "var(--procurerpa-text-muted)" }}>{block.description}</div>
          ) : null}
          {isLoopBlock && (
            <div className="space-y-2 rounded bg-slate-elevation5 px-3 py-2 text-xs">
              <div style={{ color: "var(--procurerpa-text-secondary)" }}>
                {t("workflows.iterableValues")}{" "}
                <span className="font-medium" style={{ color: "var(--procurerpa-text-primary)" }}>
                  {loopValues.length}
                </span>
              </div>
              {loopValues.length > 0 && (
                <div className="max-h-40 space-y-1 overflow-y-auto pr-1">
                  {loopValues.map((value, index) => (
                    <div key={index} style={{ color: "var(--procurerpa-text-muted)" }}>
                      <span className="mr-1" style={{ color: "var(--procurerpa-text-muted)" }}>[{index}]</span>
                      <code className="rounded bg-slate-elevation1 px-1 py-0.5 font-mono" style={{ color: "var(--procurerpa-text-secondary)" }}>
              ...
              {hasEvaluations(block.output) && block.output.evaluations ? (
                    ...
                    >
                      {evaluation.is_default ? (
                        ...
                        <div className="space-y-1">
                          <div style={{ color: "var(--procurerpa-text-muted)" }}>
                            <code className="rounded bg-slate-elevation1 px-1 py-0.5 font-mono" style={{ color: "var(--procurerpa-text-secondary)" }}>
                          ...
                          {evaluation.rendered_expression &&
                            evaluation.rendered_expression !==
                              ...
                              <div style={{ color: "var(--procurerpa-text-muted)" }}>
                                → rendered to{" "}
                                <code className="rounded bg-slate-elevation1 px-1 py-0.5 font-mono" style={{ color: "var(--procurerpa-text-primary)" }}>
                                  {evaluation.rendered_expression}
                                </code>
                              </div>
                            )}
                          <div className="flex items-center gap-2">
                            <span style={{ color: "var(--procurerpa-text-muted)" }}>evaluated to</span>
                            <span
                              className={cn(
                                "font-medium",
                                evaluation.result
                      ...
                      {evaluation.is_matched && evaluation.next_block_label && (
                        <div className="mt-1" style={{ color: "var(--procurerpa-text-muted)" }}>
                          → Executing next block:{" "}
                          <span className="font-medium" style={{ color: "var(--procurerpa-text-secondary)" }}>
                ...
                <>
                  ...
                  block.executed_branch_expression !== undefined ? (
                    <div style={{ color: "var(--procurerpa-text-secondary)" }}>
                      Condition{" "}
                      <code className="rounded bg-slate-elevation3 px-1.5 py-0.5 font-mono" style={{ color: "var(--procurerpa-text-primary)" }}>
                    ...
                    <CollapsibleTrigger asChild>
                      ...
                      >
                        <div className="flex items-center gap-1.5">
                          <ChevronRightIcon className="size-4 transition-transform group-data-[state=open]:rotate-90" style={{ color: "var(--procurerpa-text-secondary)" }} />
                          <span className="text-xs" style={{ color: "var(--procurerpa-text-primary)" }}>{`${t("workflows.iteration")} ${iterationNumber}`}</span>
                        </div>
                        <code className="max-w-[70%] truncate rounded bg-slate-elevation1 px-1 py-0.5 text-[11px]" style={{ color: "var(--procurerpa-text-secondary)" }}>
            ...
            <div className="rounded border bg-slate-elevation4 px-2 py-1.5" style={{ borderColor: "var(--glass-border)" }}>
              <CollapsibleTrigger asChild>
                <button
                  ...
                  onClick={(event) => event.stopPropagation()}
                >
                  <div className="flex items-center gap-1.5 text-xs" style={{ color: "var(--procurerpa-text-primary)" }}>
                    {childrenOpen ? (
                  ...
                  {block.executed_branch_next_block && (
                    <span className="text-[11px]" style={{ color: "var(--procurerpa-text-muted)" }}>
