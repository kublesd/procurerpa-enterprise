...
function ParameterDisplayInline({
  ...
  if (!parameters || parameters.length === 0) {
    return (
      <div className={cn("ml-8 py-4 text-sm", className)} style={{ color: "var(--procurerpa-text-muted)" }}>
              ...
              {showDescription ? (
                <div style={{ color: "var(--procurerpa-text-muted)" }}>
                  {parameter.description ? (
                    ...
                    <span style={{ color: "var(--procurerpa-text-muted)" }}>{t("workflows.noDescription")}</span>
                  )}
                </div>
              ) : null}
            </div>
